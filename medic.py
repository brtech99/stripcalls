...
class numbr(db.Model): 
    ...
    medic = db.BooleanProperty(indexed=True, default=False)
    natOffice = db.BooleanProperty(indexed=True, default=False)
...
def printList(): #create a string containing the list of entries in the database
    ...
    for n in allNum.run(limit=20):
        r=r+n.name+':'+n.phonNbr+" med="+str(n.medic)+" ref="+str(n.ref)+" adm="+str(n.admin)+" act="+str(n.active)+", "
    return r

def SendToGroup(sender, body): #create a response string which will send the body parameter to all medics, 
#                               but not to the sender
    medics = db.GqlQuery("SELECT * FROM numbr WHERE medic = TRUE AND phonNbr != '"+sender+"' AND active = TRUE")
    rtnStr = ""
    for n in medics.run(limit=20): # the 20 is an arbitraty limit, just to prevent a runaway
...
class Init(webapp.RequestHandler): #A way to initialize the database, requires admin login
    ...
    def get(self):
        ...
        for n in allGlb.run(): #delete all existing entries
            n.delete()
        n=numbr(phonNbr="7246122359", name="Brian", ucName= 'BRIAN', admin=True, super=True)
    ...
    #        if True:
            ...
            if fromVal==None or smsSid==None or sigValid==False: #So, if no from, no sid or bad signature
                ...
                if (body != None) and body[0] == '+': # This is a command
                    ...
                    if cmd=="medic":# Medic - add this number as an medic            
                        if (mbr == None) or not mbr.admin: #attempt to add oneself as medic
                            ...
                            if pMbr <> None: ##If we already have this name in the db
                                ...
                                n=numbr(phonNbr=fromVal, name=nam, ucName=nam.upper(),medic=True, ref=False, active=False)
                                n.put()
                            ...
                            if pNum == None or len(pNum)<>10 or not validate(pNum):
                                ...
                                if pMbr <> None: ##If we already have this number in the db
                                    ...
                                    n=numbr(phonNbr=pNum, name=nam, ucName=nam.upper(),medic=True, ref=False)
                                    n.put()
                        ...
                        if (mbr == None) or not mbr.admin:
                            ...
                            if pMbr == None: #do we have this number already?
                                ...
                                write('<Sms to="%s">%s, you have been removed from the USA Fencing medic database</Sms>' % (pNum, nam))
                        ...
                        if (mbr == None) or not (mbr.admin or mbr.medic):
                            write('<Sms>You are not authorized for that command</Sms>')
                        else:
                            ...
                            else: #medic inactivates himself only
                    ...
                    elif cmd=="list":# list current medics
                        if (mbr == None) or not mbr.admin:
                            write('<Sms>You are not authorized for that command</Sms>')
                        else:
                            foundIt=db.GqlQuery("SELECT * FROM numbr WHERE medic = TRUE")
                            r = "" 
                            ...
                            if pNum == None or len(pNum)<>10 or not validate(pNum):
                                ...
                                if pMbr <> None: ##If we already have this number in the db
                                    ...
                                    if pMbr != None: #do we have this name already?
                                        ...
                                        n=numbr(phonNbr=pNum, name=nam, ucName=nam.upper(),medic=False, ref=True)
                                        n.put()
                    ...
                    if (mbr <> None): # Is this from a medic or ref we know?
                        if mbr.medic:
